...
var TimelineElementsHeader = function (_Component) {
    ...
    _this.handlePeriodClick = function (time, unit) {
      if (time && unit) {
        var _this$props = _this.props,
            handleDayClick = _this$props.handleDayClick,
            showPeriod = _this$props.showPeriod;

        if (handleDayClick && unit === 'day') {
          handleDayClick(time);
        } else {
          showPeriod((0, _moment2.default)(time - 0), unit);
        }
      }
    };
  ...
  _createClass(TimelineElementsHeader, [{
    ...
    key: 'subHeaderLabel',
    value: function subHeaderLabel(time, unit, width, dayLabelRenderer, events) {
      ...
      if (unit === 'year') {
        ...
        if (dayLabelRenderer) return dayLabelRenderer({ time: time, unit: unit, width: width, events: events });
    ...
    value: function render() {
      ...
      var _props = this.props,
          ...
          dayLabelRenderer = _props.dayLabelRenderer,
          eventsByDay = _props.eventsByDay,
      ...
      (0, _calendar.iterateTimes)(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, function (time, nextTime) {
        ...
        var events = eventsByDay && eventsByDay[time.format('YYYYMMDD')];
        ...
        bottomHeaderLabels.push(_react2.default.createElement(
          ...
          _this2.subHeaderLabel(time, minUnit, labelWidth, dayLabelRenderer, events)
        ));
      });
...
TimelineElementsHeader.propTypes = {
  ...
  handleDayClick: _propTypes2.default.func,
  dayLabelRenderer: _propTypes2.default.func,
  ...
  eventsByDay: _propTypes2.default.object,
