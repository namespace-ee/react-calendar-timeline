...
const App = () => {
  ...
  const start = useMemo(() => dayjs().startOf("day").valueOf(), []);
  const end = useMemo(() => dayjs().startOf("day").add(14, "day").valueOf(), []);
  ...
  const handleItemDrag: TimelineProps["onItemDrag"] = (dragitem: OnItemDragObjectMove | OnItemDragObjectResize) => {
    ...
    console.log(
      dayjs(dragitem.time).format("dddd, MMMM DD YYYY"),
      heading,
      infoLabelTime !== infoLabelTime,
      infoLabelGroupTitle !== infoLabelGroupTitle,
      infoLabelGroupTitle
    );
    if (infoLabelTime !== newInfoLabelTime || infoLabelGroupTitle !== newInfoLabelGroupTitle) {
