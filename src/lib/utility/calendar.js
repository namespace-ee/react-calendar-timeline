...
import { arraysEqual, _get } from './generic';
 ...
 * is being stacked
 */
export function groupStack(lineHeight, item, group, groupHeight, itemIndex) {
  ...
  if (item.dimensions.stack && item.dimensions.top === null) {
    ...
    do {
      ...
      if (collidingItem != null) {
        ...
        item.dimensions.top =
          collidingItem.dimensions.top + collidingItem.dimensions.height;
        curHeight = Math.max(
          curHeight,
...
export function getItemDimensions({
  ...
  if (dimension) {
    if (item.type == 9 || item.type == 10) itemHeightRatio = 1;
    dimension.top = null;
